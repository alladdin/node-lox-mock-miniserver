...
var WSServer = function(config, http_server) {
    ...
    this.salt = new Buffer('ccf50462-e4e6-4e2e-bd4512d36e5335d7');
    this.token = new Buffer(crypto.createHash('sha256').update(crypto.randomBytes(16).toString('hex')).digest().toString('hex'));
...
WSServer.prototype.add_routes_for_config = function() {
    ...
    this.http_server.add_route('^/dev/cfg/api', function(request, response){
        that.http_server.xml_response(response, "<LL control=\"dev/cfg/api\" value=\"{'snr': '70:4A:95:21:33:41', 'version':'"+that.config.version+"'}\" Code=\"200\"/>");
    });
    this.http_server.add_route('^/jdev/cfg/api', function(request, response){
        that.http_server.json_response(response,
            "{\"LL\": { \"control\": \"dev/cfg/api\", \"value\": \"{'snr': '70:4A:95:21:33:41', 'version':'"
            + that.config.version + "'}\", \"Code\": \"200\"}}");
    });
    ...
    this._route = [
        {
            ...
            handler: function(message) {
                that.send_json_message(message, {
                    'snr': '70:4A:95:21:33:41',
                    'version': ws_server.config.version,
                });
            },
        },
        {
            path: '^jdev/sys/getkey2',
            handler: function(message) {
                that.send_json_message(message, {
                    'key': ''+ws_server.key.toString('hex'),
                    'salt': ''+ws_server.salt.toString('hex'),
                });
            },
        },
        {
        ...
        {
            path: '^jdev/sys/gettoken',
            handler: function(message) {
                var key = ws_server.key;
                var salt = ws_server.salt.toString('hex');
                var pw_hash = crypto.createHash('sha1').update(ws_server.config.password + ':' + salt).digest('hex').toUpperCase();
                var hmac = crypto.createHmac('sha1', key);
                var hash = hmac.update(ws_server.config.username + ':' + pw_hash).digest('hex');
                var base_time = Math.floor(new Date('2009-01-01').getTime() / 1000);
                var now_time = Math.floor(new Date().getTime() / 1000);


                that.logger.debug('gettoken: '+message+' === '+'dev/sys/gettoken/' + hash + '/' + ws_server.config.username);

                var gettoken_re = new RegExp('dev/sys/gettoken/' + hash + '/' + ws_server.config.username);
                if (gettoken_re.test(message)){
                    that.send_json_message(message, {
                        'token': ''+ws_server.token,
                        'validUntil': (now_time - base_time) + 1200000,
                        'tokenRights': 6664,
                        'unsecurePass': false,
                        'key': ''+ws_server.key.toString('hex'),
                    });
                }else{
                    that.send_text_message(message, 0, 401);
                }
            },
        },
        {
            path: '^jdev/sys/refreshtoken',
            handler: function(message) {
                var base_time = Math.floor(new Date('2009-01-01').getTime() / 1000);
                var now_time = Math.floor(new Date().getTime() / 1000);
                that.send_json_message(message, {
        ...
        {
            path: '^jdev/cfg/version',
            handler: function(message) {
                that.send_text_message(message, ws_server.config.version);
            },
        },
        {
            path: '^jdev/sys/version',
            handler: function(message) {
                that.send_text_message('dev/sys/version', ws_server.config.sysversion);
            },
        },
        {
...
WSConnection.prototype.send_text_message = function(control, value, code) {
    ...
    this.logger.debug('Send Text Message: ' + response_data);
    header.writeUInt32LE(response_data.length, 4);
    this.connection.sendBytes(header);
    this.connection.sendUTF(response_data);
};

WSConnection.prototype.send_json_message = function(control, value, code) {
    code = typeof code !== 'undefined' ? code : 200;
    var header = new Buffer([0x03, 0, 0, 0, 0, 0, 0, 0]);
    var response_data = new Buffer(
        JSON.stringify({ 'LL': { 'control': ''+control, 'value': value, 'code': ''+code}})
